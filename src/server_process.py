...
from lib_process import asyncio, websockets, json, threading, os, time, logging, struct
from global_vars import config, save_config
...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    client.subscribe(f"{CLIENT_TOPIC_PREFIX}/{CLIENT_GROUP}/+/{CLIENT_PUB_MESSAGE_TOPIC}")
...
async def send_audio_chunks(hw_id, audio_bytes):
    ...
    mqtt_client.publish(f"{CLIENT_TOPIC_PREFIX}/{CLIENT_GROUP}/{hw_id}/CLIENT_SUB_MESSAGE_TOPIC", total_payload,qos=0)

    # Gửi từng chunk PCM
    for seq, pcm_chunk in enumerate(chunks):
        # global_vars.player.play_media(pcm_chunk, True)  # Phát PCM trực tiếp
        mqtt_client.publish(
